...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def disrupt_remove_node_then_add_node(self):
        ...
        node = random.choice([n for n in self.cluster.nodes if n is not self.target_node])
        self.log.info(f"node with ip {node.ip_address} will be used to run commands")
        self.log.info(f"target node ip: {self.target_node.ip_address}")
